...
<script>
import { GET_COMPARE_OPPORTUNITY_SALES_BY_SALESMAN_AND_PERIOD } from '@/graphql/querys/chartsQuery';
import { colorsSalesman } from '@/constants/colorsChart';
import { linesDataMixin } from '@/mixins/charts/linesMixin';

export default {
  name: 'LineOpportunitySalesData',
  ...
  data() {
    return {
      compareSalesBySalesmanLinearGraph: {
        mainDateList: [],
  ...
  computed: {
    options() {
      return {
        ...
        xaxis: {
          categories: this.compareSalesBySalesmanLinearGraph?.mainDateList?.map(
            (list) => {
    ...
    series() {
      const mainSeries =
        this.compareSalesBySalesmanLinearGraph?.mainCompareSalesBySalesman?.compareSalesBySalesman?.map(
      ...
      const secondSeries =
        ...
        this.compareSalesBySalesmanLinearGraph?.secondCompareSalesBySalesman
          ?.compareSalesBySalesman?.length > 0
          ? this.compareSalesBySalesmanLinearGraph?.secondCompareSalesBySalesman?.compareSalesBySalesman?.map(
              ({ name, countList, sumList }) => {
                return {
      ...
      const mainAverage = [
        {
          ...
          data: this.isCount
            ? this.convertTwoDigits(
                this.compareSalesBySalesmanLinearGraph.mainAverageDateCompareSalesBySalesman.map(
                  (item) => item.avgCount,
                ),
              )
            : this.convertTwoDigits(
      ...
      const secondAverage =
        this.compareSalesBySalesmanLinearGraph
          ?.secondAverageDateCompareSalesBySalesman?.length > 0
          ? [
              {
                name: 'Promedio anterior',
                data: this.isCount
                  ? this.convertTwoDigits(
                      this.compareSalesBySalesmanLinearGraph.secondAverageDateCompareSalesBySalesman.map(
                        (item) => item.avgCount,
                      ),
                    )
                  : this.convertTwoDigits(
  ...
  methods: {
    update() {
      this.$apollo.queries.compareSalesBySalesmanLinearGraph.refetch();
    },
  },
  apollo: {
    compareSalesBySalesmanLinearGraph: {
      query: GET_COMPARE_OPPORTUNITY_SALES_BY_SALESMAN_AND_PERIOD,
