...
<script>
import { GET_OPPORTUNITIES_BY_WEEK_BARS } from '@/graphql/querys/chartsQuery';
import { colorsSalesman } from '@/constants/colorsChart';
import { linesDataMixin } from '@/mixins/charts/linesMixin';

export default {
  name: 'LineOpportunitySalesByWeekData',
  ...
  computed: {
    options() {
      return {
        ...
        xaxis: {
          categories: this.dealsLinearGraph?.mainDateList?.map((list) => {
    ...
    series() {
      const mainSeries =
        this.dealsLinearGraph?.mainDeals?.usersDeals?.map(
      ...
      const secondSeries =
        ...
        this.dealsLinearGraph?.secondDeals?.usersDeals?.length > 0
          ? this.dealsLinearGraph?.secondDeals?.usersDeals?.map(
              ({ name, countList, sumList }) => {
                return {
      ...
      const mainAverage = [
        {
          ...
          data: this.isCount
            ? this.convertTwoDigits(
                this.dealsLinearGraph.mainAverageDateDeals.map(
                  (item) => item.avgCount,
                ),
              )
            : this.convertTwoDigits(
      ...
      const secondAverage =
        this.dealsLinearGraph?.secondDeals?.length > 0
          ? [
              {
                name: 'Promedio anterior',
                data: this.isCount
                  ? this.convertTwoDigits(
                      this.dealsLinearGraph.secondAverageDateDeals.map(
                        (item) => item.avgCount,
                      ),
                    )
                  : this.convertTwoDigits(
  ...
  methods: {
    update() {
      this.$apollo.queries.dealsLinearGraph.refetch();
    },
  },
  apollo: {
    dealsLinearGraph: {
      query: GET_OPPORTUNITIES_BY_WEEK_BARS,
