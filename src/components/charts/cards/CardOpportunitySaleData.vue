<template>
  <div>
    ...
    ></BaseCardNumber>
    <CardOpportunitySaleInformation
      v-if="!isPreview"
      :information="opportunitySaleCard.informationBar"
      :filterBy="settings.filterBy"
    ></CardOpportunitySaleInformation>
  </div>
</template>
<script>
import { GET_OPPORTUNITY_SALE_CARD } from '@/graphql/querys/chartsQuery';
import { cardsDataMixin } from '@/mixins/charts/cardsMixin';
import CardOpportunitySaleInformation from '@/components/charts/cards/CardOpportunitySaleInformation.vue';

export default {
  name: 'CardOpportunitySaleData',
  ...
  computed: {
    isCompareUser() {
      return (
        this.opportunitySaleCard?.mainOpportunitySales?.id !==
          this.opportunitySaleCard?.secondOpportunitySales?.id &&
    ...
    generalConvertionRateFormat() {
      if (
        this.opportunitySaleCard &&
        this.opportunitySaleCard?.mainOpportunitySales?.count !== undefined &&
        this.opportunitySaleCard?.mainOpportunitySales?.sum !== undefined
      ) {
        if (this.settings.filterBy === 'count') {
          return this.opportunitySaleCard?.mainOpportunitySales.count || '0';
        }
        if (this.settings.filterBy === 'sum') {
          return (
            `$${this.opportunitySaleCard?.mainOpportunitySales.sum}` || '0'
          );
        }
      }
      return '0';
    ...
    generalConvertionRateCompare() {
      if (
        this.opportunitySaleCard &&
        this.opportunitySaleCard?.secondOpportunitySales?.count !== undefined &&
        this.opportunitySaleCard?.secondOpportunitySales?.sum !== undefined
      ) {
        const convertionRateComparation = this.opportunitySaleCard
          ?.secondOpportunitySales[this.settings.filterBy];
    ...
    valueCompare() {
      if (!this.isPreview) {
        if (this.isCompareUser) {
          return this.opportunitySaleCard?.secondOpportunitySales?.name;
        }
        if (this.settings.periodRangeToCompare.period === 'CUSTOM') {
    ...
    valueMain() {
      if (!this.isPreview) {
        if (this.isCompareUser) {
          return this.opportunitySaleCard?.mainmainOpportunitySalesLostDeals
            ?.name;
        }
        if (
  ...
  methods: {
    update() {
      this.$apollo.queries.opportunitySaleCard.refetch();
    },
  },
  apollo: {
    opportunitySaleCard: {
      query: GET_OPPORTUNITY_SALE_CARD,
