  ...
  <v-card flat class="mt-5">
    <v-container>
      <v-row class="justify-center">
        <v-col cols="3">
          <template v-for="(user, index) in information.userLostDeals">
...
<script>
export default {
  name: 'CardIncomeLostDealsInformation',
  props: {
    information: {
      type: Object,
      default: () => {
  ...
  computed: {
    getInfo() {
      const firstColumn = this.information.periodsLostDeals.map((item) => {
        return item.count;
      });

      const secondColumn = this.information.periodsLostDeals.map((item) => {
        return item.sum;
      });

      const thirdColumn = this.information.periodsLostDeals.map((item) => {
