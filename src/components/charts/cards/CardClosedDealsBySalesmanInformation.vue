  ...
  <v-card flat class="mt-5">
    <v-container>
      <v-row class="justify-center">
        <v-col cols="3">
          <template v-for="(user, index) in information.userClosedDeals">
...
<script>
export default {
  name: 'CardClosedDealsBySalesmanInformation',
  props: {
    information: {
      type: Object,
      default: () => {
  ...
  computed: {
    getInfo() {
      const firstColumn = this.information.periodsClosedDeals.map((item) => {
        return item.count;
      });

      const secondColumn = this.information.periodsClosedDeals.map((item) => {
        return item.sum;
      });

      const thirdColumn = this.information.periodsClosedDeals.map((item) => {
