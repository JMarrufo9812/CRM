<template>
  <div>
    ...
    ></BaseCardNumber>
    <CardIneffectiveActivitiesInformation
      v-if="!isPreview"
      :information="ineffectiveActivitiesCard.informationBar"
      :filterBy="settings.filterBy"
    ></CardIneffectiveActivitiesInformation>
  </div>
</template>
<script>
import CardIneffectiveActivitiesInformation from '@/components/charts/cards/CardIneffectiveActivitiesInformation.vue';
import { GET_INEFFECTIVE_ACTIVITIES_CARD } from '@/graphql/querys/chartsQuery';
import { cardsDataMixin } from '@/mixins/charts/cardsMixin';

export default {
  name: 'CardIneffectiveActivitiesData',
  mixins: [cardsDataMixin],
  components: { CardIneffectiveActivitiesInformation },
  props: {
    defaultFilter: {
      type: Object,
      default: () => {
  ...
  computed: {
    ...
    generalConvertionRateFormat() {
      if (
        this.ineffectiveActivitiesCard &&
        this.ineffectiveActivitiesCard?.mainIneffectiveActivities?.count !==
          undefined
      ) {
        return (
          this.ineffectiveActivitiesCard?.mainIneffectiveActivities.count || '0'
        );
      }
      return '0';
    },
    generalConvertionRateCompare() {
      if (
        this.ineffectiveActivitiesCard &&
        this.ineffectiveActivitiesCard?.secondIneffectiveActivities?.count !==
          undefined
      ) {
        return (
          this.ineffectiveActivitiesCard?.secondIneffectiveActivities.count ||
  ...
  methods: {
    update() {
      this.$apollo.queries.ineffectiveActivitiesCard.refetch();
    },
  },
  apollo: {
    ineffectiveActivitiesCard: {
      query: GET_INEFFECTIVE_ACTIVITIES_CARD,
