<template>
  <div>
    ...
    <CardRottenDealsInformation
      v-if="!isPreview"
      :information="rottenDealsCard.informationBar"
      :filterBy="settings.filterBy"
    ></CardRottenDealsInformation>
  </div>
</template>
<script>
import CardRottenDealsInformation from '@/components/charts/cards/CardRottenDealsInformation.vue';
import { GET_ROTTEN_DEALS_CARD } from '@/graphql/querys/chartsQuery';
import { cardsDataMixin } from '@/mixins/charts/cardsMixin';

export default {
  name: 'CardRottenDealsData',
  ...
  computed: {
    ...
    generalConvertionRateFormat() {
      if (
        this.rottenDealsCard &&
        this.rottenDealsCard?.mainRottenDeals?.count !== undefined &&
        this.rottenDealsCard?.mainRottenDeals?.sum !== undefined
      ) {
        return (
          this.rottenDealsCard?.mainRottenDeals[this.settings.filterBy] || '0'
        );
      }
      return '0';
    },
    generalConvertionRateCompare() {
      if (
        this.rottenDealsCard &&
        this.rottenDealsCard?.secondRottenDeals?.count !== undefined &&
        this.rottenDealsCard?.secondRottenDeals?.sum !== undefined
      ) {
        const convertionRateComparation = this.rottenDealsCard
          ?.secondRottenDeals[this.settings.filterBy];
  ...
  methods: {
    update() {
      this.$apollo.queries.rottenDealsCard.refetch();
    },
  },
  apollo: {
    rottenDealsCard: {
      query: GET_ROTTEN_DEALS_CARD,
