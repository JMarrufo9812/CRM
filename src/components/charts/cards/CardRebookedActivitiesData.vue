<template>
  <div>
    ...
    ></BaseCardNumber>
    <CardRebookedActivitiesInformation
      v-if="!isPreview"
      :information="rebookedActivitiesCard.informationBar"
      :filterBy="settings.filterBy"
    ></CardRebookedActivitiesInformation>
  </div>
</template>
<script>
import CardRebookedActivitiesInformation from '@/components/charts/cards/CardRebookedActivitiesInformation.vue';
import { GET_REBOOKED_ACTIVITIES_CARD } from '@/graphql/querys/chartsQuery';
import { cardsDataMixin } from '@/mixins/charts/cardsMixin';

export default {
  name: 'CardRebookedActivitiesData',
  mixins: [cardsDataMixin],
  components: { CardRebookedActivitiesInformation },
  props: {
    defaultFilter: {
      type: Object,
      default: () => {
  ...
  computed: {
    ...
    generalConvertionRateFormat() {
      if (
        this.rebookedActivitiesCard &&
        this.rebookedActivitiesCard?.mainRebookedActivities?.count !== undefined
      ) {
        return this.rebookedActivitiesCard?.mainRebookedActivities.count || '0';
      }
      return '0';
    },
    generalConvertionRateCompare() {
      if (
        this.rebookedActivitiesCard &&
        this.rebookedActivitiesCard?.secondRebookedActivities?.count !==
          undefined
      ) {
        return (
          this.rebookedActivitiesCard?.secondRebookedActivities.count || '0'
        );
      }
      return '0';
    },
  ...
  methods: {
    update() {
      this.$apollo.queries.rebookedActivitiesCard.refetch();
    },
  },
  apollo: {
    rebookedActivitiesCard: {
      query: GET_REBOOKED_ACTIVITIES_CARD,
